...
class BrandProduct extends Controller
{
    ...
    //endfunction admin pages
    public function show_brand_home($brand_id){
        $cate_product = DB::table('tbl_product')->where('category_status','1')->orderby('category_id','desc')->get();
        $brand_product = DB::table('tbl_brand')->where('brand_status','1')->orderby('brand_id','desc')->get();
        $brand_by_id = DB::table('tbl_products')->join('tbl_brand','tbl_products.brand_id','=','tbl_brand.brand_id')
        ->where('tbl_products.brand_id',$brand_id)->get();
